#!/usr/bin/env python3
"""
Kalshi Suspicious Activity Watchdog Runner
============================================

Monitors Kalshi prediction markets for suspicious price movements without
corresponding public news catalysts. Same detection logic as the Polymarket
watchdog, adapted for Kalshi's market structure and API.
...
Environment Variables:
    ...
    (or KALSHI_PRIVATE_KEY: Raw PEM key string)
...
class KalshiWatchdogRunner:
    """Long-running Kalshi watchdog with logging and periodic stats."""

    def __init__(self, config: KalshiWatchdogConfig, auth: Optional[KalshiAuth],
        ...
        # Logging setup
        self.log_dir = Path("logs/watchdog_kalshi")
        self.log_dir.mkdir(parents=True, exist_ok=True)

        timestamp = self.start_time.strftime("%Y%m%d_%H%M%S")
        self.log_file = self.log_dir / f"kalshi_watchdog_{timestamp}.log"
        self.stats_file = self.log_dir / f"kalshi_watchdog_stats_{timestamp}.jsonl"

        self._setup_logging()
        self.logger = logging.getLogger("kalshi_watchdog")

        # Engine
        self.engine: Optional[KalshiWatchdogEngine] = None
        self._running = False
    ...
    async def start(self):
        """Start the Kalshi watchdog."""
        self._running = True

        self.logger.info("=" * 80)
        self.logger.info("KALSHI SUSPICIOUS ACTIVITY WATCHDOG")
        self.logger.info("=" * 80)
        ...
        if self.config.watch_event_tickers:
            self.logger.info(f"Watch Events: {', '.join(self.config.watch_event_tickers)}")
        if self.config.watch_series_tickers:
            self.logger.info(f"Watch Series: {', '.join(self.config.watch_series_tickers)}")
        self.logger.info(f"Watch Categories: {', '.join(self.config.watch_categories)}")
        self.logger.info(f"Min Volume: {self.config.min_event_volume_24h:,.0f} contracts")
        ...
        self.logger.info(f"WebSocket: {'enabled' if self.auth else 'DISABLED (no auth)'}")
        ...
        # Start engine
        self.engine = KalshiWatchdogEngine(self.config, auth=self.auth)
        ...
        self.logger.info(
            ...
            f"{stats['registry']['events_watched']} events tracked"
        )
    ...
    def _log_stats_snapshot(self):
        ...
        self.logger.info(f"  WS connected: {ws.get('connected', False)}")
        self.logger.info(f"  WS messages: {ws.get('message_count', 0)}")
        self.logger.info("-" * 60)
    ...
    def _log_final_summary(self):
        ...
        self.logger.info("=" * 80)
        self.logger.info("KALSHI WATCHDOG FINAL SUMMARY")
        self.logger.info("=" * 80)
        ...
        self.logger.info(f"Alerts: logs/watchdog_kalshi/alerts_*.jsonl")
        self.logger.info("=" * 80)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Suspicious Activity Watchdog for Kalshi',
...
Examples:
  ...
  python kalshi_watchdog_runner.py --watch-events "KXIRAN-26APR01"
    ...
    parser.add_argument('--watch-events', type=str, default=None,
                       help='Comma-separated Kalshi event tickers to force-watch')
    parser.add_argument('--watch-series', type=str, default=None,
                       help='Comma-separated Kalshi series tickers to watch')
    parser.add_argument('--categories', type=str, default=None,
                       help='Comma-separated categories (default: politics,world,geopolitics)')
    parser.add_argument('--min-volume', type=float, default=5000.0,
                       help='Minimum event volume in contracts (default: 5000)')
    ...
    parser.add_argument('--no-ws', action='store_true', default=False,
                       help='Run without WebSocket (REST polling only)')
    parser.add_argument('--api-key', type=str, default=None,
                       help='Kalshi API key ID (overrides KALSHI_API_KEY env var)')
    parser.add_argument('--private-key-path', type=str, default=None,
                       help='Path to RSA private key PEM (overrides KALSHI_PRIVATE_KEY_PATH)')
    ...
    if args.watch_events:
        config.watch_event_tickers = [
            s.strip() for s in args.watch_events.split(",") if s.strip()
        ]

    if args.watch_series:
        config.watch_series_tickers = [
            s.strip() for s in args.watch_series.split(",") if s.strip()
        ]

    if args.categories:
        config.watch_categories = [
            c.strip() for c in args.categories.split(",") if c.strip()
        ]

    # Load auth (optional — WebSocket requires it)
    auth = None
    if not args.no_ws:
        api_key = args.api_key or os.environ.get("KALSHI_API_KEY")
        key_path = args.private_key_path or os.environ.get("KALSHI_PRIVATE_KEY_PATH")
        key_str = os.environ.get("KALSHI_PRIVATE_KEY")

        if api_key and (key_path or key_str):
            try:
                if key_path:
                    auth = KalshiAuth.from_file(api_key, key_path)
                else:
                    auth = KalshiAuth.from_string(api_key, key_str)
                print(f"Kalshi auth loaded (key: {api_key[:8]}...)")
            except Exception as e:
                print(f"Warning: Failed to load Kalshi auth: {e}")
                print("Running without WebSocket (REST polling only)")
        else:
            print("No Kalshi credentials found. Set KALSHI_API_KEY + KALSHI_PRIVATE_KEY_PATH")
            print("Running without WebSocket (REST polling only)")

    runner = KalshiWatchdogRunner(config=config, auth=auth, duration_hours=args.duration)
    ...
    try:
        ...
        print("\n\nKalshi watchdog interrupted by user")
        await runner.stop()
